...
Target names follow the format: `protocol<version>_<function>_c`.

```bash
# Example: Build only the Protocol 2.0 Read/Write example
make protocol2_0_read_write_c

# Run
./protocol2_0_read_write_c
```
...
### Prerequisites
*   Ensure the Dynamixel SDK C library (`dxl_c`) is built and installed (or built within the same solution).
...
5.  In **Solution Explorer**, right-click the example project you want to run (e.g., `protocol2_0_read_write_c`).
