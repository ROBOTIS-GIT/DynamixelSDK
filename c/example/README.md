...
The examples depend on the Dynamixel SDK C library (`libdxl_c`). You must build and install it first.
...
To avoid conflicts with C++ targets in unified builds, the CMake target names for C examples end with `_c`, but the actual generated executable filenames are clean (without `_c`).

| Make Target Name (Build) | Output Executable Name (Run) | Description |
| :--- | :--- | :--- |
| `read_write_protocol2_c` | `read_write_protocol2` | Protocol 2.0 Read/Write |
| `ping_protocol2_c` | `ping_protocol2` | Protocol 2.0 Ping |
| `scan_dynamixel_c` | `scan_dynamixel` | Network Scan |
| ... | ... | ... |

**Example: Building a specific example only**
```bash
# 1. Build using the target name (with _c)
make read_write_protocol2_c

# 2. Run the executable (without _c)
...
3.  **Generate the Solution**:
    ...
    ```
4.  Open the generated `dynamixel_sdk_c_examples.sln` (or `.slnx`) file in Visual Studio.
5.  In **Solution Explorer**, right-click the example project you want to run (e.g., `read_write_protocol2_c`).
...
If you encounter an error like `error while loading shared libraries: libdxl_c.so`, ensure the library path is known to the system.
